...
### 5n. Jugar online: Premier privado y motor propio (2026-09-08)

Nel aprobó Premier casual BO1, 1 contra 1, salas por código, mazos de su cuenta y tablero móvil. La implementación vive en `feature/jugar-online`; la aprobación de esta beta no publica un servicio nuevo. La tarjeta de jugador, HomePage, Mesa/SWUSIM, laboratorio, torneos y estadísticas se conservan.

`services/juego/` es un proceso Node 24 independiente, con Socket.IO y SQLite (`node:sqlite`). Forceteki queda fijado por `motor-lock.json` a `139c14a240b75498b403ccdb06144a136972ecb6`, con catálogo normalizado oficial y SHA-256. `preparar-motor` verifica y compila en `.motor/`, gitignorado. No arranca GameServer, AWS, NextAuth ni servicios privados de Karabast. Conservar THIRD_PARTY_NOTICES/MIT. El pin y la legalidad Premier requieren mantenimiento: se rechazan cartas no implementadas y mazos ilegales; no se promete cubrir todas las interacciones futuras.

El servidor obtiene identidad y perfil mediante Supabase Auth con el token de la sesión y clave anon. Los cuerpos no eligen usuario. Dos asientos, una sala activa por cuenta, comandos en cola por sala y UUID con payload inmutable. Reintentar una respuesta perdida conserva UUID/cuerpo. Cada emisión Socket.IO y cada GET entrega la vista del usuario, nunca la mano o mazo rival completo. Revalidar Auth y reservar el cupo de socket ANTES de esperar la cola: doce admisiones simultáneas deben aceptar solo cinco. No inferir derrota por desconexión.

El motor valida comandos contra el prompt actual y el actor. Los argumentos nativos de `menuButton` pueden ser números aunque algunos tipos upstream digan string. Un prompt con `perCardButtons` exige `perCardMenuButton`: aceptar su cardUuid en `menuButton` causaba una excepción real al resolver Improvise LAW242. La regresión comprueba rechazo sin mutación y que la elección válida aún funciona. Los nombres visibles de perfiles no identifican ganadores; se usan IDs únicos.

SQLite persiste solo metadatos/resultados, nunca manos, mazos ni tokens. Un proceso por archivo; no cluster ni réplicas. Una recarga del navegador recupera el asiento mientras vive el motor. Reiniciar el servicio interrumpe salas activas sin ganador; no reconstruye partidas en curso. No hay ranking ni XP en esta beta. Operación, límites, Docker, volumen, configuración y apagado están en `services/juego/README.md`. Sin SQL ni migraciones nuevas de Supabase.

El cliente está en `src/features/jugar/`. `/jugar`, `/jugar/sala/:codigo` y `/jugar/partida/:codigo` son lazy. `VITE_JUEGO_URL` es el origen HTTPS del servicio, o loopback HTTP en desarrollo; vacío muestra disponibilidad pendiente. Carga mazos cloud filtrados por user_id, no los mazos Dexie mezclados entre perfiles. La conversión resuelve IDs legacy/UUID y variantes mediante identidad del catálogo; importar JSON es explícito. `loadFullDatabase()` verifica catálogo completo: `ensureCards()` solo asegura alguna carta y devuelve false también cuando ya existe, por lo que no sirve para decidir integridad/error.

El cliente de socket obtiene token fresco antes de cada intento y anuncia conectado solo después de GET válido. Los cierres remotos también reintentan; `io server disconnect` no usa autoreconexión nativa. Reintentos y respuestas tardías se cancelan con generación, AbortController, listeners y cierre de transporte. Cambiar cuenta remonta la sesión. Al pulsar Jugar otra se invalida el cliente y remonta una generación interna incluso si la URL sigue siendo `/jugar`: una respuesta de la partida anterior no debe restaurarla.

El tablero interpreta estado `unknown`, deriva selecciones y comandos de los prompts, muestra mano propia y zonas públicas, permite ampliar cartas con Sheet y pausa acciones sin conexión. Diez vistas reales y dieciséis contratos de prompt incluyen selección múltiple/ordenada, menú por carta, números, listas y distribución. En móvil la mano queda junto al prompt; arenas en dos columnas y bases/líderes compactos. El arte pasa por `/api/img` y conserva atribución; textos dinámicos de habilidades pueden permanecer en inglés.

Banco DEV `/banco-jugar`: tres identidades ficticias aisladas, dos mazos compatibles y el mismo cliente/lobby/tablero. No crea cuentas ni escribe Supabase. `npm run dev:jugar` agrega el proxy público de imágenes que Vite normal no ejecuta. Servicio, caché, datos y fixtures no forman parte del bundle/precache; `.vercelignore` excluye el servicio y documentación raíz del upload frontend.

Verificación: Node 24, build y lint completos (0 errores, 6 advertencias previas); 29 pruebas de motor/servicio sin omisiones; programas mazos/cliente/vistas y 7 integraciones Socket.IO. Navegador con dos usuarios: crear/unirse/listos, mano y recursos, unidades tierra/espacio, habilidad Han, ronda, ataque a base (2 daño), ampliación, recarga, concesión y nueva partida. Viewport móvil 390×844 y revisión de escritorio 1280×900; no equivale a medir rendimiento en un teléfono físico. Revisiones independientes de motor, servidor y frontend cerraron los hallazgos antes de la entrega. Desplegar requiere alojar el proceso y volumen, configurar Auth/orígenes/HTTPS y VITE_JUEGO_URL; un preview de Vercel por sí solo no habilita partidas.

### 5o. Contador visual con bases para Premier y Twin Suns (2026-09-12)

`src/features/calculadora/` ocupa el acceso principal «Contador de daños». `/contador` y `/contador/mesa` están fuera de AppLayout para usar toda la pantalla; el segundo prepara Twin Suns. `/calculadora` redirige a `/contador`. No envolver estas rutas en AuthGate: la inicialización de autenticación depende de AppLayout. Los duelos clásicos siguen dentro del layout en `/contador/registrado` y `/contador/registrado/mesa`, accesibles desde preparación/ajustes. Amistosas y Misiones de jugar/ganar apuntan al duelo registrado, que conserva sus escrituras y registros. El contador visual es manual y local, sin conceder XP ni resultados en nube.

Cada jugador puede elegir una base real al preparar la partida. `SelectorBaseCalculadora` consulta bases válidas del catálogo Dexie; verifica `isDatabaseComplete()` y completa una caché parcial con `loadFullDatabase({ force: true })` sin ocultar las bases guardadas. «Actualizar catálogo» permite refrescar incluso una caché completa. El servicio absorbe errores: escuchar su progreso y verificar el sello final evita informar éxito falso. `ensureCards()` no garantiza catálogo completo. Se filtran variantes marcadas no canónicas y vidas inválidas; búsqueda por nombre, subtítulo, expansión y número, con paginación. No usar el administrador oficial de cartas. Imágenes mediante CardImage, en horizontal. El diálogo del selector vive fuera del formulario y sus botones no lo envían.

`BaseCalculadora` conserva ID, nombre, imagen y vida impresa. La elección fija la vida inicial/máxima y bloquea el campo manual; «Usar vida manual» permite retirarla. Elegir una base solo modifica el borrador hasta iniciar una nueva partida. No intercambiar bases durante una partida. Arte y nombre se muestran en cada panel, y la carta completa en el diálogo de vida. Los guardados previos de `holocron-calculadora-casual-v1` sin base se leen con base nula; historial, recarga y deshacer preservan metadatos. El estado clásico `contador_duelo_v1` permanece separado.

«Siguiente ronda · Restablecer fichas» mantiene la vida e iniciativa, libera Plan/Explosión y reinicia las reclamaciones. Reclamar fichas no aplica automáticamente sus efectos. En Twin Suns, una base eliminada solo revive con deshacer y la primera eliminación activa la fase final. No cerrar una ronda adicional desde esa fase. Las animaciones respetan movimiento reducido y pausan con la pestaña oculta; sonido opcional, wake lock y fullscreen con limpieza al salir.

Verificación: `scripts/calculadora-estado.test.mts`, `scripts/calculadora-bases.test.mts`, `npm run misiones`, `scripts/actualizacion-rutas.test.mjs`, build y lint. Navegador con catálogo real de 91 bases: vidas 25/33, cuatro bases independientes, imágenes cargadas, selección sin envío, guardado/deshacer, catálogo sin conexión, caché parcial con una sola base y recuperación tras fallo de red, rutas y acceso al duelo registrado. Viewports 320×568, 390×844, 812×375 y 1280×900, incluida eliminación en móvil pequeño. Son pruebas de navegador; no una medición en teléfono físico.


### 5p. Fuerza en Premier y fondo de bases (2026-09-12)

Nel autorizó publicar PR #4 con fichas de Fuerza y arte de fondo más visible. `BaseCalculadora.usaFuerza` proviene de `baseUsaFuerza`, que lee creación de la ficha en `text`/`epicAction`. No inferir por HP ni por nombre: el catálogo validado tiene 12 bases canónicas LOF 19–30, cuatro de 25 y ocho de 28 HP. El selector debe transferir `usaFuerza` al guardar la elección. Solo los snapshots antiguos sin ese campo usan el conjunto de esos 12 UUID verificados; tanto estado actual como historial se normalizan, sin eliminar la partida ni crear una ficha automáticamente.

`JugadorCalculadora.fuerza` es booleano, ausente antiguo → false. `cambiarFuerza(mesa,id,boolean)` es explícito e idempotente: solo Premier, jugador vivo y base compatible; máximo una ficha por jugador. Crear/gastar escribe historial, recarga y Deshacer preservan estado. `siguienteRonda` conserva Fuerza; derrota elimina la ficha y Deshacer restaura. Fuerza es independiente de reclamar iniciativa. No aplicar condiciones/efectos de cartas automáticamente: por ejemplo Mystic Monastery tiene un límite de tres activaciones de su habilidad, que se resuelve en la mesa física, no un límite global de fichas creadas por otras cartas. Reglas: CR 8.37 local y [presentación oficial](https://starwarsunlimited.com/articles/legends-of-the-force).

El botón aparece en el pie de cada base compatible, con estado Sin ficha/Disponible, acción accesible y feedback visual/sonoro opcional. `calc-base-arte` da presencia al arte, recorta la zona ilustrada y conserva velos de contraste locales; el orbe anima solo opacidad, respetando efectos apagados, pestaña oculta y movimiento reducido. Mantener Fuerza accesible en paneles estrechos/cortos. Las pruebas de calculadora incluyen detección por habilidad, migración, booleanos inválidos, crear/gastar, independencia, ronda, KO y Deshacer. Pruebas de navegador con imágenes reales, tamaños móviles/horizontales y escritorio; no se afirma rendimiento medido en teléfono físico.


### 5q. Contador: sugerencias de usuarios, pulsación sostenida y audio (2026-09-12)

`NombreJugadorCalculadora` permite buscar perfiles visibles en cada asiento de Premier/Twin Suns al escribir al menos dos caracteres, con debounce, cancelación, descarte de respuestas antiguas y límite de espera. Reutiliza `searchProfiles` con opciones aditivas de AbortSignal y error explícito; los llamadores anteriores conservan el contrato de devolver [] ante fallos. El cliente Supabase recupera su sesión persistida sin llamar initAuth del shell. No añadir esa inicialización aquí: también inicia sincronización de colección, visitas y otras acciones ajenas al contador.

La lista vive en el flujo bajo la fila: `.calc-preparacion-jugador` tiene overflow:hidden, por lo que un dropdown absoluto se recortaría. Las opciones usan Avatar, botones type=button y navegación combobox/listbox; Enter selecciona sin iniciar partida y Escape cierra. La selección solo completa el nombre, respetando el máximo de 32 caracteres y los IDs locales de asiento. Nombres libres siguen válidos. No afirmar cuenta vinculada ni registrar estadísticas desde el contador manual. Respetar los perfiles que la política existente permita consultar; sin cambios RLS ni SQL.


`BotonVidaCalculadora` controla ±1/±5: toque al soltar, repetición tras 350ms y luego cada 140ms sin acelerar, sin un click final extra. Mantiene callback actual por ref y cancela timer/captura al salir del área, soltar/cancelar puntero, perder foco/captura, ocultar la pestaña, desmontar o deshabilitarse al alcanzar límites. Conservar click de detail=0 para teclado/tecnología de asistencia y no contar doble los clicks físicos. No permitir que un hold siga cambiando una partida detrás de un diálogo. La mesa y botones deshabilitan selección de texto/menú táctil; los inputs siguen editables.

`sonidoCalculadora.sonarConsola` usa dos osciladores breves, filtro y envolvente con ataque para daño/curación distintos. Armónicos medios hacen el golpe más perceptible que el seno anterior de baja frecuencia. Desconecta cada voz al terminar; useConsola limita golpes a uno cada 60ms y conserva cierre/suspensión de AudioContext. Opciones antiguas reciben volumen80 manteniendo sonido/efectos/orientación. Ajustes permite 0–100, guarda localmente y ofrece probar el golpe; no activa sonido automáticamente. Render OfflineAudioContext: RMS de daño .0092→.0271 por pulso a volumen80; pico a100 inferior a.28, sin clipping. No equivale a medir el altavoz físico del usuario.

...
function Avatar({ avatar }: { avatar: string }) {
  ...
  return (
    <img
      ...
      className={`w-20 h-20 ${isPhotoAvatar(avatar) ? 'object-cover rounded-full' : 'object-contain'}`}
    />
  )
}
...
export function TarjetaJugador({
  ...
  const cuerpo = (
    <>
      <div className="flex items-center gap-3 mb-3">
        <ProfileFrame level={nivel?.level || 1} size={size}>
          <div className="w-full h-full flex items-center justify-center overflow-hidden">
            <Avatar avatar={perfil.avatar || 'darth-vader'} />
          </div>
        </ProfileFrame>
        <div className="flex-1 min-w-0">
          <h2 className="text-lg font-extrabold text-swu-text truncate">
            {perfil.name || 'Jugador'}
          </h2>
          <div className="flex items-center gap-2 flex-wrap">
            {nivel && (
              <span className={`text-[11px] font-bold ${nivel.rank.color}`}>{nivel.rank.name}</span>
            )}
            {pais && (
  ...
  const marco = 'bg-gradient-to-br from-swu-accent/15 to-amber-500/10 rounded-2xl p-4 border border-swu-accent/20'

  if (alTocar === 'nada') return <div className={marco}>{cuerpo}</div>

  return (
    <button
      onClick={() => navigate('/profile')}
      aria-label="Ver mi perfil"
      className={`${marco} w-full text-left active:scale-[0.99] transition-transform

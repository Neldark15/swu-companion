...
export function DeckBuilderPage() {
  ...
  const addCardToDeck = (card: Card) => {
    // Auto-detect target based on card type
    if (card.type === 'Leader' || card.isLeader) {
      const existing = deck.leaders.find((c) => c.cardId === card.id)
      if (existing) return // already have this leader
      const rules = deck.format === 'twin_suns' ? 2 : 1
      if (deck.leaders.length >= rules) return

      setDeck((prev) => ({
        ...prev,
        leaders: [...prev.leaders, { cardId: card.id, name: card.name, subtitle: card.subtitle, quantity: 1, setCode: card.setCode }],
      }))
      return
    }
    ...
    if (card.type === 'Base' || card.isBase) {
      setDeck((prev) => ({
        ...prev,
        base: { cardId: card.id, name: card.name, subtitle: card.subtitle, quantity: 1, setCode: card.setCode },
      }))
      return
    }
    ...
    setDeck((prev) => {
      ...
      if (idx >= 0) {
        ...
        list.push({ cardId: card.id, name: card.name, subtitle: card.subtitle, quantity: 1, setCode: card.setCode })
      }
  ...
  const removeCard = (target: 'leaders' | 'mainDeck' | 'sideboard' | 'base', cardId: string) => {
    if (target === 'base') {
      setDeck((prev) => ({ ...prev, base: null }))
      return
    }
    if (target === 'leaders') {
      setDeck((prev) => ({ ...prev, leaders: prev.leaders.filter((c) => c.cardId !== cardId) }))
      return
    }
    setDeck((prev) => {
  ...
  return (
    <div className="p-3 space-y-3 pb-24">
      ...
      <div className="flex items-center justify-between">
        <button onClick={() => navigate('/decks')} className="flex items-center gap-1 text-sm text-swu-muted">
          <ChevronLeft size={18} /> Decks
        </button>
        <div className="flex gap-2">
          <button
            onClick={saveDeck}
            className={`p-2 rounded-lg border transition-colors ${
              saveFlash ? 'bg-swu-green/20 border-swu-green/40 text-swu-green' : 'bg-swu-surface border-swu-border text-swu-muted'
            }`}
          >
      ...
      <div className="flex items-center gap-2">
        {editingName ? (
          <input
            autoFocus
            className="flex-1 bg-swu-surface border border-swu-border rounded-lg px-3 py-2 text-sm text-swu-text font-bold"
          ...
          <button onClick={() => setEditingName(true)} className="flex-1 text-left">
            <h2 className="text-lg font-bold text-swu-text">{deck.name}</h2>
          </button>
        )}
        <select
          value={deck.format}
          onChange={(e) => setDeck((p) => ({ ...p, format: e.target.value as TournamentFormat | 'limited' }))}
      ...
      <div className={`flex items-center gap-2 rounded-lg px-3 py-2 border ${
        deck.isValid ? 'bg-swu-green/10 border-swu-green/30' : 'bg-swu-amber/10 border-swu-amber/30'
      }`}>
        {deck.isValid ? (
          <CheckCircle2 size={14} className="text-swu-green" />
        ) : (
          <AlertTriangle size={14} className="text-swu-amber" />
        )}
        <span className={`text-xs font-bold ${deck.isValid ? 'text-swu-green' : 'text-swu-amber'}`}>
          {deck.isValid ? 'Deck válido' : deck.validationErrors[0] || 'Deck incompleto'}
        </span>
      ...
      <div className="flex bg-swu-surface rounded-xl border border-swu-border overflow-hidden">
        <button
          ...
          className={`flex-1 py-2.5 flex items-center justify-center gap-1.5 text-xs font-bold transition-colors ${
            tab === 'deck' ? 'bg-swu-accent text-white' : 'text-swu-muted'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2.5 flex items-center justify-center gap-1.5 text-xs font-bold transition-colors ${
            tab === 'search' ? 'bg-swu-accent text-white' : 'text-swu-muted'
          }`}
        >
          <Search size={14} /> Buscar Cartas
        </button>
      </div>
      ...
      {tab === 'deck' && (
        <div className="space-y-3">
          {/* Leader */}
          <div>
            <p className="text-xs font-bold text-swu-amber mb-1.5">Leader ({deck.leaders.length})</p>
            {deck.leaders.length === 0 ? (
              <p className="text-[10px] text-swu-muted bg-swu-surface rounded-lg p-3 border border-swu-border text-center">
                Busque un Leader para agregarlo
              </p>
            ) : (
              deck.leaders.map((c) => (
                <div key={c.cardId} className="bg-swu-surface rounded-lg p-2.5 border border-swu-amber/30 flex items-center justify-between">
                  <div>
                    <span className="text-sm font-bold text-swu-text">{c.name}</span>
                    {c.subtitle && <span className="text-xs text-swu-muted ml-1.5">{c.subtitle}</span>}
                  </div>
          ...
          <div>
            ...
            {!deck.base ? (
              <p className="text-[10px] text-swu-muted bg-swu-surface rounded-lg p-3 border border-swu-border text-center">
                Busque una Base para agregarla
              </p>
            ) : (
              <div className="bg-swu-surface rounded-lg p-2.5 border border-swu-green/30 flex items-center justify-between">
                <div>
                  <span className="text-sm font-bold text-swu-text">{deck.base.name}</span>
                  {deck.base.subtitle && <span className="text-xs text-swu-muted ml-1.5">{deck.base.subtitle}</span>}
                </div>
          ...
          <div>
            ...
            {deck.mainDeck.length === 0 ? (
              <p className="text-[10px] text-swu-muted bg-swu-surface rounded-lg p-3 border border-swu-border text-center">
                Vaya a "Buscar Cartas" para agregar
              </p>
            ) : (
              <div className="space-y-1">
                {deck.mainDeck.map((c) => (
                  <div key={c.cardId} className="bg-swu-surface rounded-lg px-2.5 py-2 border border-swu-border flex items-center gap-2">
                    <span className="w-6 h-6 rounded bg-swu-accent/20 text-swu-accent text-xs font-bold flex items-center justify-center font-mono">
                      {c.quantity}
                    </span>
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => removeCard('mainDeck', c.cardId)} className="w-6 h-6 rounded bg-swu-red/10 text-swu-red flex items-center justify-center">
                        <Minus size={12} />
                      </button>
                      <button onClick={() => incrementCard('mainDeck', c.cardId)} className="w-6 h-6 rounded bg-swu-green/10 text-swu-green flex items-center justify-center">
                        <Plus size={12} />
                      </button>
                    </div>
                  </div>
          ...
          <div>
            ...
            {deck.sideboard.length === 0 ? (
              <p className="text-[10px] text-swu-muted bg-swu-surface rounded-lg p-3 border border-swu-border text-center">
                Opcional — agregue cartas desde "Buscar Cartas"
              </p>
            ) : (
              <div className="space-y-1">
                {deck.sideboard.map((c) => (
                  <div key={c.cardId} className="bg-swu-surface rounded-lg px-2.5 py-2 border border-swu-border flex items-center gap-2">
                    <span className="w-6 h-6 rounded bg-purple-400/20 text-purple-400 text-xs font-bold flex items-center justify-center font-mono">
                      {c.quantity}
                    </span>
                    <div className="flex-1 min-w-0">
                      <span className="text-sm text-swu-text truncate block">{c.name}</span>
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => removeCard('sideboard', c.cardId)} className="w-6 h-6 rounded bg-swu-red/10 text-swu-red flex items-center justify-center">
                        <Minus size={12} />
                      </button>
                      <button onClick={() => incrementCard('sideboard', c.cardId)} className="w-6 h-6 rounded bg-swu-green/10 text-swu-green flex items-center justify-center">
                        <Plus size={12} />
                      </button>
                    </div>
                  </div>
      ...
      {tab === 'search' && (
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => setAddTarget('mainDeck')}
              className={`flex-1 py-2 rounded-lg text-xs font-bold border transition-colors ${
                addTarget === 'mainDeck' ? 'bg-swu-accent/20 border-swu-accent text-swu-accent' : 'bg-swu-surface border-swu-border text-swu-muted'
              }`}
            >
              Mazo ({mainCount})
            </button>
            <button
              ...
              className={`flex-1 py-2 rounded-lg text-xs font-bold border transition-colors ${
                addTarget === 'sideboard' ? 'bg-purple-400/20 border-purple-400 text-purple-400' : 'bg-swu-surface border-swu-border text-swu-muted'
              }`}
            >
              Sideboard ({sideCount})
            </button>
                ...
                {searchResults.map((card) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={isLeaderInDeck || isBaseInDeck}
                    ...
                    >
                      {card.imageUrl && (
                        <img src={card.imageUrl} alt="" className="w-8 h-11 rounded object-cover bg-swu-bg flex-shrink-0" loading="lazy" />
                      )}
                      <div className="flex-1 min-w-0">
                      ...
                      <div className="flex-shrink-0 text-right">
                        {(inDeck || inSide) && (
                          <span className="text-[10px] text-swu-accent font-bold">
                            ×{(inDeck?.quantity || 0) + (inSide?.quantity || 0)}
                          </span>
                        )}
                        {isLeaderInDeck && <span className="text-[9px] text-swu-amber">Leader</span>}
                        {isBaseInDeck && <span className="text-[9px] text-swu-green">Base</span>}
                        {!isLeaderInDeck && !isBaseInDeck && (
                          <Plus size={16} className="text-swu-accent" />
                        )}
                      </div>
                    </button>
                  )
          ...
          {!searching && searchQuery && searchResults.length === 0 && (
            <div className="text-center py-8">
              <BookOpen size={28} className="mx-auto text-swu-muted/40 mb-2" />
              <p className="text-xs text-swu-muted">No se encontraron cartas</p>
            </div>
          )}

          {!searchQuery && (
            <div className="text-center py-8">
              <Search size={28} className="mx-auto text-swu-muted/40 mb-2" />
              <p className="text-xs text-swu-muted">Escriba el nombre de una carta</p>
              <p className="text-[10px] text-swu-muted mt-1">Leaders y Bases se asignan automáticamente</p>
            </div>
          )}
        </div>
      )}

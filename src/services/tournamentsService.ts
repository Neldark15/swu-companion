...
import { db } from './db'
import { normalizeSearch } from './swuApi'
...
function readCache<T>(key: string): T | null {
  try {
    ...
    if (v !== CACHE_VERSION || Date.now() - at > TTL) return null
    return data
  } catch {
    return null
  }
...
/** Se construye una sola vez: el mapeo corre sobre ~165 torneos de golpe. */
function getIndex(): Promise<CardIndex> {
